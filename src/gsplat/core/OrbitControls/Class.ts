import { Vector3, Matrix3, Quaternion, Camera } from "gsplat";

/*
  Пришлось достать OrbitControls из исходников библиотеки т.к. огиринальный OrbitControls
  не позволяет иземенять скорость изменении камеры при работе через клавиатуру.

  Так же, оригинальный OrbitControls прячет в себе св-во нажатых клавиш. 
  Тут состояние нажатых клавишь достал наружу. 
  Так удобнее дебажить и можно напрямую привязать состояние интерфейса к состоянию OrbitControls.
*/
...
class OrbitControls {
  ...
  constructor(
    ...
    this.update = () => {
      ...
      const moveSpeed = this.panSpeed * 0.025;
      const rotateSpeed = this.orbitSpeed * 0.01;
